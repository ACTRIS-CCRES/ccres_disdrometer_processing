"""Create daily quicklooks from the preprocessed file format.

Input : output of the preprocessing = daily file
Output : 2 Panels of daily quicklooks :
    * 2D DCR data,
    * disdrometer information
    * weather variables if provided
"""
...
def daily_quicklooks(preprocessed_file):
    ...
    (ax1, ax2, ax3, ax4, ax5, ax6) = (  # noqa F841
        axes[0, 0],
        axes[0, 1],
        axes[1, 0],
        axes[1, 1],
    ...
    fig.text(
        s=pd.Timestamp(ds.time.values[0]).to_pydatetime().strftime("%Y-%m-%d")
        + " : Daily data overview",
        fontsize=18,
    ...
    ax1.set_title(f"DCR {ds.radar_frequency * 1e-9:.0f} GHz reflectivity")
    ...
    ax2.set_title(f"DCR {ds.radar_frequency * 1e-9:.0f} Doppler Velocity")
    ...
    ax3b = ax3.twinx()
    temperature = ax3.plot(
        ds.time, ds["ta"].values, label=ds["ta"].attrs["long_name"], color="red"
    )
    ax3.set_ylabel(f"{ds['ta'].attrs['long_name']} ({ds['ta'].attrs['units']})")

    humidity = ax3b.plot(
        ds.time, ds["hur"].values, label=ds["hur"].attrs["long_name"], color="green"
    )
    ...
    ax4b = ax4.twinx()
    wind_speed = ax4.plot(  # noqa
        ds.time, ds["ws"].values, label=ds["ws"].attrs["long_name"], color="red"
    )
    ax4.set_ylabel(f"{ds['ws'].attrs['long_name']} ({ds['ws'].attrs['units']})")

    wind_direction = ax4b.plot(  # noqa
        ds.time, ds["wd"].values, label=ds["wd"].attrs["long_name"], color="green"
    )
    ...
    # Disdrometer-related data :
    # rain accumulation and 2D plot for the Speed/Diameter relationship
    ax5.plot(ds.time, ds["disdro_cp"], color="red", label="Disdrometer")
    ax5.plot(
        ds.time[:],
        ds["ams_cp"][:],
        color="green",
    ...
    ax5.set_title(
        "Disdrometer and weather station-based cumulated precipitation over the day"
    )
    ax5.grid()
...
if __name__ == "__main__":
    ...
    fig.savefig(
        f"{TEST_OUT_DIR}/20210202_palaiseau_test_quicklooks.png",
        dpi=500,
        transparent=False,
    )
