...
def main_loop_degrade(config_file=CONFIG_FILE_LOOP):  # Juelich Mira : 35GHz, not 95
    if CONF:
        config = toml.load(config_file)
        # print(config)
        disdro_files = (
    ...
    for year, month in zip(years, months):
        ...
        for d in range(1, 32):
            ...
            output_file = output_files.format(year, month, day)
            print("HELLO")
            print((os.path.exists(disdro_file)))
            if os.path.exists(disdro_file):
                # read and preprocess disdrometer data
                # ---------------------------------------------------------------------------------

                if disdro_type == "parsivel_cloudnet":
                    disdro_xr = disdro.read_parsivel_cloudnet(disdro_file)
                elif disdro_type == "parsivel_cloudnet_juelich":
                    disdro_xr = disdro.read_parsivel_cloudnet_juelich(disdro_file)
                scatt = dcrcc.scattering_prop(
                    disdro_xr.size_classes[0:-5],
                    beam_orientation,
                    FREQ_J,
...
def main_loop_quicklooks(config_file=CONFIG_FILE_LOOP):
    ...
    # 2 = parcourir les dates d'événement et créer les fichiers radar, disdro(prepro) et WS à ces dates
    # for start, end in zip(start_events, end_events) :
    # get_data_event(config.DATA_DIR, config.STATION, start, end, thresholds=TIMESTAMP_THRESHOLDS + EVENT_THRESHOLDS)
    # quicklooks(weather,dcr,disdro,output_path,thresholds=TIMESTAMP_THRESHOLDS + EVENT_THRESHOLDS,)
