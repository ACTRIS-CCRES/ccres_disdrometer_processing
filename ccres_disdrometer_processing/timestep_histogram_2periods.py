...
def histos(data, p1=t1, p2=t2) :
    ...
    for i in range(2) :
        ...
        ax.hist(
            ...
            color = colors[i],
            density=True,
            label="{} - {}, {} timesteps".format(dt.datetime.strptime(ps[i][0], "%Y/%m/%d %H:%M:%S").strftime("%Y/%m/%d"), dt.datetime.strptime(ps[i][1],"%Y/%m/%d %H:%M:%S").strftime("%Y/%m/%d"), len(df))
        )
        median_dz = np.nanmedian(df.iloc[:, 1] - df.iloc[:, 0])
        ax.axvline(
            x=median_dz,
            color = colors[i],
            label=r"Median $\Delta Z$ = {:.2f} dBZ, $\sigma$ = {:.2f} dBZ".format(median_dz, np.nanstd(df.iloc[:, 1] - df.iloc[:, 0])),
        )
        ax.grid()
    ...
    plt.savefig('/homedata/ygrit/disdro_processing/juelich/timeseries2/histo_compare_2periods_example.png', transparent=False, dpi=500)
    plt.show()
